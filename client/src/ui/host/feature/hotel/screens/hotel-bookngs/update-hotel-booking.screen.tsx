...
const HostUpdateHotelBookings: React.FC = () => {
    ...
    const handleSubmit = async (values: IStatusBookingHotel) => {
        try {
            ...
            await api.put(`/hotel-bookings/${id}`, values).then(response=>{
                ...
                if(response.data?.status==='accepted') {
                    ...
                    api.post('/hotel-reservations',{
                        hotel: bookingHotel?.hotel?._id,
                        checkIn: bookingHotel?.startDate,
                        checkOut: bookingHotel?.endDate,
                        totalPrice: bookingHotel?.totalPrice,
                        rooms: bookingHotel?.rooms,
                        travelers: bookingHotel?.travelers,
                        customer: bookingHotel?.user._id,
                    })
                }
            });

            // Redirect after successful update
            navigate(`/host/booking-hotels/${bookingHotel?.hotel?._id}`);
        } catch (error) {
...
export default HostUpdateHotelBookings;
