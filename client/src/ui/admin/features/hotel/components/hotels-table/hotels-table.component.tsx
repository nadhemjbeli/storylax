// src/ui/admin/features/hotel/components/hotels-table.component.tsx
import React, { useState } from 'react';
import './hotels-table.style.scss';
import { IHotel } from '../../../../../../data/hotel/hotel.data.ts';
import { ReactComponent as ServicesIcon } from '../../../../../../assets/svg/admin/hotel/services.icon.svg';
...
const HotelTable: React.FC<HotelTableProps> = ({ hotels, onDelete, itemsPerPage = 10 }) => {
    ...
    const filteredHotels = hotels.filter((hotel) => {
        ...
        const hostMatch = `${(hotel.host as any)?.firstName} ${(hotel.host as any)?.lastName}`.toLowerCase().includes(searchQuery);
    ...
    const sortedHotels = [...filteredHotels].sort((a, b) => {
        ...
        } else if (sortConfig.column === 'host') {
            comparison = (a.host?.firstName || '').localeCompare(b.host?.firstName || '');
                    ...
                    <tr key={hotel._id} className="table-row">
                        ...
                        <td className="table-data">
                            <div className="table-buttons">
                                <Link to={`/admin/booking-hotels/${hotel._id}`}>
                                    <button className="bookings-button">
                                        <BookingsIcon className="admin-manage-icon" />
                                    </button>
                                </Link>
                                <Link to={`/admin/hotel-services/${hotel._id}`}>
                                    <button className="services-button">
                                        <ServicesIcon className="admin-manage-icon" />
                                    </button>
                                </Link>
